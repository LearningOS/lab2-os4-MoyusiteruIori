...
impl TaskManager {
    ...
    pub fn translate_in_current_task(&self, va: VirtAddr) -> usize {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
    ...
    pub fn update_syscall_times(&self, syscall_id: usize) {
        ...
        current_task_syscall_times[syscall_id] += 1;
        drop(inner);
    }
    ...
    pub fn get_current_task_syscall_times(&self) -> [u32; MAX_SYSCALL_NUM] {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let current_task_syscall_times = inner.tasks[current].syscall_times.clone();
        drop(inner);
