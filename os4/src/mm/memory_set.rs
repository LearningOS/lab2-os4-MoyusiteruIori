...
impl MemorySet {
    ...
    pub fn contains_vpn(&self, target: &VirtPageNum) -> bool {
        for ma in self.areas.iter() {
    ...
    pub fn munmap(&mut self, start: VirtAddr, end: VirtAddr) -> isize {
        ...
        for vpn in vpnrange {
            if let Some(pte) = self.page_table.find_pte(vpn) {
                if pte.bits == PageTableEntry::empty().bits {
                    return -1;
                }
            }
            else {
        ...
        for vpn in vpnrange {
            for area in self.areas.iter_mut() {
                if let Some(_) = area.data_frames.remove(&vpn) {
                    area.unmap_one(&mut self.page_table, vpn);
                    // 这个 vpn 已经被解映射了，可以继续下一个 vpn 了
                    break;
                }
            }
        }
